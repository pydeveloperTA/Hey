...
def haversine_meters(lat1, lon1, lat2, lon2):
    """Great-circle distance in meters. Used as a fallback speed source,
    since some devices/providers report GPS speed as 0 or omit it entirely."""
    R = 6371000.0
...
class NavigationScreen(DynamicScreen):
    def __init__(self, **kwargs):
        ...
        self.mosque_markers = []
        self.prayer_times = {}
        # Auto-follow: map recenters on GPS updates until the user drags it,
        # then it stays put until they tap the recenter button.
        self.auto_follow = True
        self._suppress_follow_break = False
        ...
        # Any lat/lon change we didn't trigger ourselves means the user
        # dragged the map, so pause auto-follow until they tap recenter.
        self.map_view.bind(lat=self._on_map_dragged, lon=self._on_map_dragged)
        ...
        # Recenter button
        self.recenter_btn = SmoothButton(text="📍", size_hint=(None,None), size=(dp(44),dp(44)), pos_hint={'right':0.98, 'y':0.18}, font_size=sp(20), radius=22)
        self.recenter_btn.bind(on_release=self.recenter_map)
        main_layout.add_widget(self.recenter_btn)

        # "Follow paused" hint - shown only after the user pans away
        self.follow_hint_lbl = Label(text="Tap 📍 to resume tracking", font_size=sp(11), bold=True,
                                      color=(1,1,1,1), size_hint=(None,None), size=(dp(200), dp(32)),
                                      pos_hint={'center_x':0.5, 'y':0.115})
        with self.follow_hint_lbl.canvas.before:
            self.follow_hint_color = Color(0.95,0.55,0.15,0.95)
            self.follow_hint_rect = RoundedRectangle(size=self.follow_hint_lbl.size, pos=self.follow_hint_lbl.pos, radius=[dp(16)])
        self.follow_hint_lbl.bind(size=self._sync_follow_hint, pos=self._sync_follow_hint)
        self.follow_hint_lbl.opacity = 0
        main_layout.add_widget(self.follow_hint_lbl)
    ...
    def _sync_follow_hint(self, instance, value):
        self.follow_hint_rect.size = instance.size; self.follow_hint_rect.pos = instance.pos

    def _on_map_dragged(self, *args):
        """Fires on any lat/lon change - including our own recenters, which
        is why those go through _programmatic_center's suppress flag."""
        if self._suppress_follow_break or not self.dest_lat:
            return
        if self.auto_follow:
            self.auto_follow = False
            self.follow_hint_lbl.opacity = 1

    def _programmatic_center(self, lat, lon):
        self._suppress_follow_break = True
        self.map_view.center_on(lat, lon)
        Clock.schedule_once(lambda dt: setattr(self, '_suppress_follow_break', False), 0.05)

    def recenter_map(self, instance):
        self.auto_follow = True
        self.follow_hint_lbl.opacity = 0
        if self.current_lat and self.current_lon:
            self._programmatic_center(self.current_lat, self.current_lon)
    ...
    def recalculate_route(self):
        ...
        def on_route_calculated(dist_km, dur_mins, sync_str, polyline_coords):
            ...
            arrival = datetime.datetime.now() + datetime.timedelta(minutes=dur_mins)
            arrival_str = arrival.strftime("%I:%M %p").lstrip("0")
            self.dest_name_lbl.text = f"Target: {self.dest_name}"
            self.metrics_lbl.text = f"{dist_km} km  •  {time_str}  •  arrives {arrival_str}  ({cardinal_str})"
            if polyline_coords:
    ...
    def on_gps_location(self, **kwargs):
        lat = kwargs.get('lat')
        lon = kwargs.get('lon')
        provider_speed = kwargs.get('speed', None)
        if not lat or not lon:
            return

        now = time.time()
        computed_kmh = None
        if self._last_gps_pos and self._last_gps_time:
            plat, plon = self._last_gps_pos
            dt = now - self._last_gps_time
            if dt > 0.3:
                dist_m = haversine_meters(plat, plon, lat, lon)
                computed_kmh = (dist_m / dt) * 3.6
        self._last_gps_pos = (lat, lon)
        self._last_gps_time = now

        # Prefer the device-reported speed when it looks real. A lot of
        # providers leave it at 0 (or omit it) even while you're moving, so
        # fall back to a distance/time estimate from consecutive fixes.
        if provider_speed is not None and provider_speed > 0.3:
            kmh = provider_speed * 3.6
        elif computed_kmh is not None:
            kmh = computed_kmh
        else:
            kmh = 0.0

        self._speed_history.append(kmh)
        if len(self._speed_history) > 4:
            self._speed_history.pop(0)
        smoothed_kmh = sum(self._speed_history) / len(self._speed_history)

        self.current_lat = lat
        self.current_lon = lon
        self.user_layer.set_position(lat, lon)
        app = App.get_running_app()
        if app.touch_lock_enabled and self.auto_follow:
            self._programmatic_center(lat, lon)

        self.speed_unit_lbl.text = "km/h"
        self.speed_val_lbl.text = str(int(round(smoothed_kmh)))
